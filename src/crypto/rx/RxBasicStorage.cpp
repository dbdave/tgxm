...
{
public:
    TGXM_DISABLE_COPY_MOVE(RxBasicStoragePrivate)
...
tgxm::RxBasicStorage::RxBasicStorage() :
...
bool tgxm::RxBasicStorage::isAllocated() const
{
    return d_ptr->dataset() && d_ptr->dataset()->cache() && d_ptr->dataset()->cache()->get();
}


tgxm::HugePagesInfo tgxm::RxBasicStorage::hugePages() const
{
    if (!d_ptr->dataset()) {
        return {};
    }
...
tgxm::RxDataset *tgxm::RxBasicStorage::dataset(const Job &job, uint32_t) const
{
...
void tgxm::RxBasicStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    d_ptr->setSeed(seed);
