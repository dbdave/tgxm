...
tgxm::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
tgxm::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


tgxm::RxCache::~RxCache()
{
...
bool tgxm::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
tgxm::HugePagesInfo tgxm::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void tgxm::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
