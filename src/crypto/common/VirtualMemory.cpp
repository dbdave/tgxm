...
tgxm::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
...
tgxm::VirtualMemory::~VirtualMemory()
{
    if (!m_scratchpad) {
        return;
    }
...
tgxm::HugePagesInfo tgxm::VirtualMemory::hugePages() const
{
    return { this };
}


#ifndef TGXM_FEATURE_HWLOC
uint32_t tgxm::VirtualMemory::bindToNUMANode(int64_t)
{
    return 0;
}
#endif
...
void tgxm::VirtualMemory::destroy()
{
    delete pool;
}


void tgxm::VirtualMemory::init(size_t poolSize, size_t hugePageSize)
{
    if (!pool) {
        osInit(hugePageSize);
    }
