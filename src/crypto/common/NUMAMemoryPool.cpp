...
tgxm::NUMAMemoryPool::NUMAMemoryPool(size_t size, bool hugePages) :
...
tgxm::NUMAMemoryPool::~NUMAMemoryPool()
{
    for (auto kv : m_map) {
        delete kv.second;
    }
...
bool tgxm::NUMAMemoryPool::isHugePages(uint32_t node) const
{
    if (!m_size) {
        return false;
    }
...
uint8_t *tgxm::NUMAMemoryPool::get(size_t size, uint32_t node)
{
    if (!m_size) {
        return nullptr;
    }
...
void tgxm::NUMAMemoryPool::release(uint32_t node)
{
...
tgxm::IMemoryPool *tgxm::NUMAMemoryPool::get(uint32_t node) const
{
    return m_map.count(node) ? m_map.at(node) : nullptr;
}


tgxm::IMemoryPool *tgxm::NUMAMemoryPool::getOrCreate(uint32_t node) const
{
    auto pool = get(node);
    if (!pool) {
