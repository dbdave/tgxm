...
tgxm::MemoryPool::MemoryPool(size_t size, bool hugePages, uint32_t node)
{
    if (!size) {
        return;
    }
...
tgxm::MemoryPool::~MemoryPool()
{
    delete m_memory;
}


bool tgxm::MemoryPool::isHugePages(uint32_t) const
{
    return m_memory && m_memory->isHugePages();
}


uint8_t *tgxm::MemoryPool::get(size_t size, uint32_t)
{
...
void tgxm::MemoryPool::release(uint32_t)
{
    assert(m_refs > 0);
