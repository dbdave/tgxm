...
tgxm::Benchmark::Benchmark(size_t workers, const IBackend *backend) :
    m_backend(backend),
    m_workers(workers)
{
}
...
bool tgxm::Benchmark::finish(uint64_t totalHashCount)
{
...
void tgxm::Benchmark::start()
{
    m_startTime = BenchState::start(m_workers, m_backend);
}


void tgxm::Benchmark::printProgress() const
{
