...
tgxm::CudaBaseRunner::CudaBaseRunner(size_t id, const CudaLaunchData &data) :
    m_data(data),
    m_threadId(id)
{
}
...
tgxm::CudaBaseRunner::~CudaBaseRunner()
{
    CudaLib::release(m_ctx);
}


bool tgxm::CudaBaseRunner::init()
{
...
bool tgxm::CudaBaseRunner::set(const Job &job, uint8_t *blob)
{
    m_height = job.height();
    m_target = job.target();
...
size_t tgxm::CudaBaseRunner::intensity() const
{
    return m_data.thread.threads() * m_data.thread.blocks();
}


bool tgxm::CudaBaseRunner::callWrapper(bool result) const
{
    if (!result) {
