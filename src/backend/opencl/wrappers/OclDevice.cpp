...
tgxm::OclDevice::OclDevice(uint32_t index, cl_device_id id, cl_platform_id platform) :
    m_id(id),
    m_platform(platform),
...
tgxm::String tgxm::OclDevice::printableName() const
{
...
uint32_t tgxm::OclDevice::clock() const
{
    return OclLib::getUint(id(), CL_DEVICE_MAX_CLOCK_FREQUENCY);
}


void tgxm::OclDevice::generate(const Algorithm &algorithm, OclThreads &threads) const
{
...
void tgxm::OclDevice::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
