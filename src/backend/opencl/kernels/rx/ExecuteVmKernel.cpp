...
void tgxm::ExecuteVmKernel::enqueue(cl_command_queue queue, size_t threads, size_t worksize)
{
...
void tgxm::ExecuteVmKernel::setArgs(cl_mem vm_states, cl_mem rounding, cl_mem scratchpads, cl_mem dataset_ptr, uint32_t batch_size)
{
...
void tgxm::ExecuteVmKernel::setFirst(uint32_t first)
{
    setArg(6, sizeof(uint32_t), &first);
}


void tgxm::ExecuteVmKernel::setIterations(uint32_t num_iterations)
{
...
void tgxm::ExecuteVmKernel::setLast(uint32_t last)
{
