...
tgxm::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool tgxm::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value tgxm::OclThread::toJSON(rapidjson::Document &doc) const
{
