...
tgxm::OclSharedData &tgxm::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void tgxm::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void tgxm::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
