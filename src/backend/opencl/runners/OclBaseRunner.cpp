...
tgxm::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef TGXM_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
tgxm::OclBaseRunner::~OclBaseRunner()
{
...
size_t tgxm::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t tgxm::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void tgxm::OclBaseRunner::build()
{
...
cl_mem tgxm::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t tgxm::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void tgxm::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void tgxm::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void tgxm::OclBaseRunner::finalize(uint32_t *hashOutput)
{
