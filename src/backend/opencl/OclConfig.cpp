...
#ifndef TGXM_OS_APPLE
tgxm::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
tgxm::OclConfig::OclConfig() = default;
#endif


tgxm::OclPlatform tgxm::OclConfig::platform() const
{
...
rapidjson::Value tgxm::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<tgxm::OclLaunchData> tgxm::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void tgxm::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void tgxm::OclConfig::generate()
{
    ...
    count += tgxm::generate<Algorithm::CN>(m_threads, devices);
    count += tgxm::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += tgxm::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += tgxm::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += tgxm::generate<Algorithm::CN_FEMTO>(m_threads, devices);
    count += tgxm::generate<Algorithm::RANDOM_X>(m_threads, devices);
    count += tgxm::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void tgxm::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef TGXM_OS_APPLE
void tgxm::OclConfig::setPlatform(const rapidjson::Value &platform)
{
