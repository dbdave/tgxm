...
bool tgxm::HttpData::isJSON() const
{
...
const char *tgxm::HttpData::methodName() const
{
    return llhttp_method_name(static_cast<llhttp_method>(method));
}


rapidjson::Document tgxm::HttpData::json() const
{
    if (status < 0) {
...
const char *tgxm::HttpData::statusName(int status)
{
    if (status < 0) {
