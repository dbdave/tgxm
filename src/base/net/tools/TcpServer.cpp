...
static const tgxm::String kLocalHost("127.0.0.1");


tgxm::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
tgxm::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int tgxm::TcpServer::bind()
{
    if (!m_version) {
...
void tgxm::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void tgxm::TcpServer::onConnection(uv_stream_t *stream, int status)
{
