...
tgxm::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


tgxm::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool tgxm::ServerTls::isHTTP(const char *data, size_t size)
{
    assert(size > 0);
...
bool tgxm::ServerTls::isTLS(const char *data, size_t size)
{
    assert(size > 0);
...
bool tgxm::ServerTls::send(const char *data, size_t size)
{
...
void tgxm::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
