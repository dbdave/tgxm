...
tgxm::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
tgxm::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t tgxm::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void tgxm::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void tgxm::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void tgxm::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool tgxm::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void tgxm::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void tgxm::SelfSelectClient::retry()
{
    setState(RetryState);
}


void tgxm::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void tgxm::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void tgxm::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void tgxm::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
