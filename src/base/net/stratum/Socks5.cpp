...
tgxm::Client::Socks5::Socks5(Client *client) :
    m_client(client)
{
}


bool tgxm::Client::Socks5::read(const char *data, size_t size)
{
    if (size < m_nextSize) {
        return false;
    }
...
void tgxm::Client::Socks5::handshake()
{
    m_nextSize  = 2;
...
bool tgxm::Client::Socks5::isIPv4(const String &host, sockaddr_storage *addr)
{
    return uv_ip4_addr(host.data(), 0, reinterpret_cast<sockaddr_in *>(addr)) == 0;
}


bool tgxm::Client::Socks5::isIPv6(const String &host, sockaddr_storage *addr)
{
   return uv_ip6_addr(host.data(), 0, reinterpret_cast<sockaddr_in6 *>(addr)) == 0;
}


void tgxm::Client::Socks5::connect()
{
    m_nextSize  = 5;
