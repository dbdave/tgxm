...
tgxm::Url::Url(const char *url)
{
    parse(url);
}


tgxm::Url::Url(const char *host, uint16_t port, bool tls, Scheme scheme) :
    m_tls(tls),
    m_scheme(scheme),
    m_host(host),
    m_port(port)
...
bool tgxm::Url::parseIPv6(const char *addr)
{
