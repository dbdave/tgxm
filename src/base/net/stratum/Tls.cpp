...
tgxm::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool tgxm::Client::Tls::handshake(const char* servername)
{
    m_ssl = SSL_new(m_ctx);
...
bool tgxm::Client::Tls::send(const char *data, size_t size)
{
...
const char *tgxm::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *tgxm::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void tgxm::Client::Tls::read(const char *data, size_t size)
{
...
bool tgxm::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool tgxm::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool tgxm::Client::Tls::verifyFingerprint(X509 *cert)
{
