...
bool tgxm::Pools::isEqual(const Pools &other) const
{
...
int tgxm::Pools::donateLevel() const
{
#   ifdef TGXM_FEATURE_BENCHMARK
...
tgxm::IStrategy *tgxm::Pools::createStrategy(IStrategyListener *listener) const
{
    if (active() == 1) {
...
rapidjson::Value tgxm::Pools::toJSON(rapidjson::Document &doc) const
{
...
size_t tgxm::Pools::active() const
{
    size_t count = 0;
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
...
void tgxm::Pools::load(const IJsonReader &reader)
{
    m_data.clear();
...
uint32_t tgxm::Pools::benchSize() const
{
#   ifdef TGXM_FEATURE_BENCHMARK
...
void tgxm::Pools::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void tgxm::Pools::setDonateLevel(int level)
{
...
void tgxm::Pools::setProxyDonate(int value)
{
    switch (value) {
...
void tgxm::Pools::setRetries(int retries)
{
...
void tgxm::Pools::setRetryPause(int retryPause)
{
