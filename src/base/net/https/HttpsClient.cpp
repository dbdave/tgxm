...
tgxm::HttpsClient::HttpsClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
const char *tgxm::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *tgxm::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void tgxm::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void tgxm::HttpsClient::read(const char *data, size_t size)
{
...
void tgxm::HttpsClient::write(std::string &&data, bool close)
{
...
bool tgxm::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool tgxm::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void tgxm::HttpsClient::flush(bool close)
{
