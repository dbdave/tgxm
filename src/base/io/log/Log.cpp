...
{
public:
    TGXM_DISABLE_COPY_MOVE(LogPrivate)
...
void tgxm::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void tgxm::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void tgxm::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
