...
tgxm::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
void tgxm::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void tgxm::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
