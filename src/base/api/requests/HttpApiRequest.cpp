...
tgxm::HttpApiRequest::HttpApiRequest(const HttpData &req, bool restricted) :
...
bool tgxm::HttpApiRequest::accept()
{
...
const rapidjson::Value &tgxm::HttpApiRequest::json() const
{
    if (type() == REQ_JSON_RPC) {
...
tgxm::IApiRequest::Method tgxm::HttpApiRequest::method() const
{
    return static_cast<IApiRequest::Method>(m_req.method);
}


void tgxm::HttpApiRequest::done(int status)
{
...
void tgxm::HttpApiRequest::setRpcError(int code, const char *message)
{
    using namespace rapidjson;
    auto &allocator = doc().GetAllocator();
...
void tgxm::HttpApiRequest::setRpcResult(rapidjson::Value &result)
{
    rpcDone(kResult, result);
}


void tgxm::HttpApiRequest::rpcDone(const char *key, rapidjson::Value &value)
{
    ApiRequest::done(0);
