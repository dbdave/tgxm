...
static inline void random_scalar(ec_scalar& res)
{
    ...
    tgxm::Cvt::randomBytes(res.data, sizeof(res.data));
...
{
    tgxm::keccak((const uint8_t*) data, length, (uint8_t*) &res, sizeof(res));
...
bool generate_key_derivation(const uint8_t* key1, const uint8_t* key2, uint8_t* derivation, uint8_t* view_tag)
{
    ...
    if (view_tag) {
        ...
        tgxm::keccak(buf, sizeof(buf), view_tag_full, sizeof(view_tag_full));
