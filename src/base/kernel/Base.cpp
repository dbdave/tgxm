...
{
public:
    TGXM_DISABLE_COPY_MOVE_DEFAULT(BasePrivate)
...
private:
    inline static Config *load(Process *process)
    {
        ...
        chain.addFile(Process::location(Process::HomeLocation, ".config" TGXM_DIR_SEPARATOR APP_ID ".json"));
        if (read(chain, config)) {
            return config.release();
        }
...
tgxm::Base::Base(Process *process)
...
bool tgxm::Base::isBackground() const
{
    return d_ptr->config && d_ptr->config->isBackground();
}


bool tgxm::Base::reload(const rapidjson::Value &json)
{
...
tgxm::Config *tgxm::Base::config() const
{
...
void tgxm::Base::addListener(IBaseListener *listener)
{
    d_ptr->listeners.push_back(listener);
}


void tgxm::Base::onFileChanged(const String &fileName)
{
...
#ifdef TGXM_FEATURE_API
void tgxm::Base::onRequest(IApiRequest &request)
{
