...
static const char *kDonateHost = "donate.v2.tgxm.com";
#ifdef TGXM_FEATURE_TLS
static const char *kDonateHostTls = "donate.ssl.tgxm.com";
#endif

} // namespace tgxm


tgxm::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
...
tgxm::DonateStrategy::~DonateStrategy()
{
    delete m_timer;
    delete m_strategy;
...
void tgxm::DonateStrategy::update(IClient *client, const Job &job)
{
...
int64_t tgxm::DonateStrategy::submit(const JobResult &result)
{
    return m_proxy ? m_proxy->submit(result) : m_strategy->submit(result);
}


void tgxm::DonateStrategy::connect()
{
    m_proxy = createProxy();
    if (m_proxy) {
        m_proxy->connect();
...
void tgxm::DonateStrategy::setAlgo(const tgxm::Algorithm &algo)
{
    m_algorithm = algo;
...
void tgxm::DonateStrategy::setProxy(const ProxyUrl &proxy)
{
    m_strategy->setProxy(proxy);
}


void tgxm::DonateStrategy::stop()
{
    m_timer->stop();
    m_strategy->stop();
}
...
void tgxm::DonateStrategy::tick(uint64_t now)
{
    m_now = now;
...
void tgxm::DonateStrategy::onActive(IStrategy *, IClient *client)
{
    if (isActive()) {
        return;
    }
...
void tgxm::DonateStrategy::onPause(IStrategy *)
{
}


void tgxm::DonateStrategy::onClose(IClient *, int failures)
{
...
void tgxm::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void tgxm::DonateStrategy::onLogin(IStrategy *, IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    setParams(doc, params);
}


void tgxm::DonateStrategy::onLoginSuccess(IClient *client)
{
    if (isActive()) {
        return;
    }
...
void tgxm::DonateStrategy::onVerifyAlgorithm(const IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void tgxm::DonateStrategy::onVerifyAlgorithm(IStrategy *, const  IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void tgxm::DonateStrategy::onTimer(const Timer *)
{
    setState(isActive() ? STATE_WAIT : STATE_CONNECT);
}


tgxm::IClient *tgxm::DonateStrategy::createProxy()
{
...
void tgxm::DonateStrategy::idle(double min, double max)
{
    m_timer->start(random(m_idleTime, min, max), 0);
}


void tgxm::DonateStrategy::setJob(IClient *client, const Job &job, const rapidjson::Value &params)
{
    if (isActive()) {
...
void tgxm::DonateStrategy::setParams(rapidjson::Document &doc, rapidjson::Value &params)
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void tgxm::DonateStrategy::setResult(IClient *client, const SubmitResult &result, const char *error)
{
    m_listener->onResultAccepted(this, client, result, error);
}


void tgxm::DonateStrategy::setState(State state)
{
