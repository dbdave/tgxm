...
{
public:
    TGXM_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void tgxm::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void tgxm::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void tgxm::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void tgxm::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, miner_signature));
}


void tgxm::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(TGXM_FEATURE_OPENCL) || defined(TGXM_FEATURE_CUDA)
void tgxm::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
