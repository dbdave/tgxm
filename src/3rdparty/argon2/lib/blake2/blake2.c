...
void tgxm_ar2_blake2b_init(blake2b_state *S, size_t outlen)
{
...
void tgxm_ar2_blake2b_update(blake2b_state *S, const void *in, size_t inlen)
{
...
void tgxm_ar2_blake2b_final(blake2b_state *S, void *out, size_t outlen)
{
    ...
    tgxm_ar2_clear_internal_memory(buffer, sizeof(buffer));
    tgxm_ar2_clear_internal_memory(S->buf, sizeof(S->buf));
    tgxm_ar2_clear_internal_memory(S->h, sizeof(S->h));
}

void tgxm_ar2_blake2b_long(void *out, size_t outlen, const void *in, size_t inlen)
{
    ...
    if (outlen <= BLAKE2B_OUTBYTES) {
        tgxm_ar2_blake2b_init(&blake_state, outlen);
        tgxm_ar2_blake2b_update(&blake_state, outlen_bytes, sizeof(outlen_bytes));
        tgxm_ar2_blake2b_update(&blake_state, in, inlen);
        tgxm_ar2_blake2b_final(&blake_state, pout, outlen);
    } else {
        uint32_t toproduce;
        uint8_t out_buffer[BLAKE2B_OUTBYTES];

        tgxm_ar2_blake2b_init(&blake_state, BLAKE2B_OUTBYTES);
        tgxm_ar2_blake2b_update(&blake_state, outlen_bytes, sizeof(outlen_bytes));
        tgxm_ar2_blake2b_update(&blake_state, in, inlen);
        tgxm_ar2_blake2b_final(&blake_state, out_buffer, BLAKE2B_OUTBYTES);
        ...
        while (toproduce > BLAKE2B_OUTBYTES) {
            tgxm_ar2_blake2b_init(&blake_state, BLAKE2B_OUTBYTES);
            tgxm_ar2_blake2b_update(&blake_state, out_buffer, BLAKE2B_OUTBYTES);
            tgxm_ar2_blake2b_final(&blake_state, out_buffer, BLAKE2B_OUTBYTES);
        ...
        tgxm_ar2_blake2b_init(&blake_state, toproduce);
        tgxm_ar2_blake2b_update(&blake_state, out_buffer, BLAKE2B_OUTBYTES);
        tgxm_ar2_blake2b_final(&blake_state, out_buffer, toproduce);

        memcpy(pout, out_buffer, toproduce);

        tgxm_ar2_clear_internal_memory(out_buffer, sizeof(out_buffer));
    }
}
