...
int argon2_ctx_mem(argon2_context *context, argon2_type type, void *memory,
    ...
    int result = tgxm_ar2_validate_inputs(context);
    uint32_t memory_blocks, segment_length;
     ...
     * blocks
     */
    result = tgxm_ar2_initialize(&instance, context);
    ...
    /* 4. Filling memory */
    result = tgxm_ar2_fill_memory_blocks(&instance);

    if (ARGON2_OK != result) {
        return result;
    }
    /* 5. Finalization */
    tgxm_ar2_finalize(context, &instance);
...
int argon2_hash(const uint32_t t_cost, const uint32_t m_cost,
    ...
    if (result != ARGON2_OK) {
        tgxm_ar2_clear_internal_memory(out, hashlen);
        free(out);
        return result;
    }
    ...
    if (encoded && encodedlen) {
        if (encode_string(encoded, encodedlen, &context, type) != ARGON2_OK) {
            tgxm_ar2_clear_internal_memory(out, hashlen); /* wipe buffers if error */
            tgxm_ar2_clear_internal_memory(encoded, encodedlen);
            free(out);
