...
Option `coin` useful for pools without [algorithm negotiation](https://tgxm.com/docs/extensions/algorithm-negotiation) support or daemon to allow automatically switch algorithm in next hard fork. If you use tgxm-proxy don't need specify algorithm on miner side.
...
## Migration to v3
Since version 3 mining [algorithm](#algorithm-names) should specified for each pool separately (`algo` option), earlier versions was use one global `algo` option and per pool `variant` option (this option was removed in v3). If your pool support [mining algorithm negotiation](https://github.com/tgxm/tgxm-proxy/issues/168) you may not specify this option at all.
